...
import { CreateUserRequest, UpdateMyInfoRequest } from "../dto/usersDTO";
...
export const createUserHandler = async (
  req: Request<{}, UserKoResponse, CreateUserRequest>,
  res: Response<UserKoResponse>
): Promise<void> => {
  ...
  res
    ...
    .json(
      toUserKoResponse(
        user as Prisma.UserGetPayload<{ include: { company: true } }>
      )
    );
};
...
export const getMyInfoHandler = async (
  ...
  res: Response<UserKoResponse | { message: string }>
): Promise<void> => {
  if (!req.user) {
